...
def heightfield(mapfile):
  heightmap = cv.imread(mapfile,0)
  return cv.blur(heightmap,(5,5))
...
### Changed ###
heightfieldSource = useTerrainFromJpg
...
if heightfieldSource==useTerrainFromJpg:
  mapfile = "/home/ethan/Documents/valkyrie/PyPnC/maps/kierran-want-height-map.jpg"
  heightmap = cv.imread(mapfile,0)
  heightmap = cv.blur(heightmap,(5,5))
  numHeightfieldColumns = heightmap.shape[0]
  numHeightfieldRows = heightmap.shape[1]
  heightfieldData = heightmap.flatten()
  terrainShape = p.createCollisionShape(shapeType = p.GEOM_HEIGHTFIELD, meshScale=[.05,.05,.005], heightfieldTextureScaling=1, heightfieldData=heightfieldData, numHeightfieldRows=numHeightfieldRows, numHeightfieldColumns=numHeightfieldColumns)
...
if heightfieldSource==useTerrainFromPNG:
  terrainShape = p.createCollisionShape(shapeType = p.GEOM_HEIGHTFIELD, meshScale=[.1,.1,24],fileName = "heightmaps/kierran-want-height-map.jpg")
  textureId = p.loadTexture("heightmaps/kierran-want-height-map.jpg")
  terrain  = p.createMultiBody(0, terrainShape)
  p.changeVisualShape(terrain, -1, textureUniqueId = textureId)
